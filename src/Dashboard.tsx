...
import { Avatar, Button, EmptyState, Money, StatCard, fmtMoney } from './primitives';
...
interface DashboardProps {
  data: AppData;
  layout?: 'cards' | 'list';
  onNav?: (page: Page, opts?: { openNew?: boolean }) => void;
  onMarkPaid?: (id: string) => void;
}

export const Dashboard = ({ data, layout = 'cards', onNav, onMarkPaid }: DashboardProps) => {
  ...
  const stats = { recebido, pendente, despesas, saldo, inadimplentes };
    ...
    <>
      <div className="page-header">
        ...
        <div className="actions">
          <Button variant="secondary" icon={Icons.Download}>Exportar mês</Button>
          <Button icon={Icons.Plus} onClick={() => onNav?.('movimentacoes', { openNew: true })}>Nova movimentação</Button>
        </div>
      </div>

      {layout === 'cards'
        ? <DashCardsLayout data={data} stats={stats} chartMax={chartMax} proximos={proximos} ultimas={ultimas} onMarkPaid={onMarkPaid} />
        : <DashListLayout data={data} stats={stats} chartMax={chartMax} proximos={proximos} ultimas={ultimas} onMarkPaid={onMarkPaid} />
      }
    </>
  );
};
...
interface LayoutProps {
  data: AppData;
  stats: { recebido: number; pendente: number; despesas: number; saldo: number; inadimplentes: number };
  chartMax: number;
  proximos: AppData['movs'];
  ultimas: AppData['movs'];
  onMarkPaid?: (id: string) => void;
}

const DashCardsLayout = ({ data, stats, chartMax, proximos, ultimas, onMarkPaid }: LayoutProps) => {
    ...
    <>
      <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr 1fr 1fr', gap: 16, marginBottom: 20 }}>
        <StatCard
          ...
          sub={<><Icons.TrendUp size={12} />+12,4% vs. outubro</>}
        />
        <StatCard
          label="Recebido"
...
// ---- LIST LAYOUT ----
const DashListLayout = ({ data, stats, chartMax, proximos, onMarkPaid }: LayoutProps) => {
  const { recebido, pendente, despesas, saldo, inadimplentes } = stats;
      ...
      <div className="hero-card">
        <div className="hero-grid">
          <div>
            ...
            <div className="hero-trend"><Icons.TrendUp size={14} />+12,4% vs. outubro</div>
          </div>
          <div>
...
// ---- Sub-cards ----
const ChartCard = ({ chartMax }: { chartMax: number }) => (
...
const ProximosCard = ({ items, data, onMarkPaid }: { items: AppData['movs']; data: AppData; onMarkPaid?: (id: string) => void }) => (
  <div className="card card-pad-0">
    <div className="card-header">
    ...
    <div>
      ...
      {items.length === 0 && <EmptyState icon={Icons.CheckCircle} title="Tudo em dia." body="Nenhuma cobrança em aberto." />}
    </div>
  </div>
);

const UltimasCard = ({ items, data }: { items: AppData['movs']; data: AppData }) => (
  <div className="card card-pad-0">
    <div className="card-header">
...
const InadimplentesCard = ({ data }: { data: AppData }) => {
  ...
  return (
    <div className="card card-pad-0">
      ...
      <div>
        ...
        {inadimplentes.length === 0 && <EmptyState icon={Icons.Trophy} title="Time inteiro em dia." body="Nenhum mensalista atrasado este mês." />}
      </div>
    </div>
  );
};
