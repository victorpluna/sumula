// Login screen — split panel: form on left, brand panel on right.

const Login = ({ onLogin }) => {
  const [mode, setMode] = React.useState("login");
  const [email, setEmail] = React.useState("carlos@realvilamariana.com");
  const [password, setPassword] = React.useState("realvila2025");
  ...
  return (
    <div style={{ minHeight: "100vh", display: "grid", gridTemplateColumns: "1fr 1fr" }}>
        ...
        <div style={{ width: "100%", maxWidth: 380 }}>
          ...
          {mode === "login" ? (
              ...
              <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                ...
                <Field label="Senha">
                  <div style={{ position: "relative" }}>
                    ...
                    <button type="button" onClick={() => setShow(!show)} style={{ position: "absolute", right: 8, top: 7, background: "transparent", border: "none", color: "var(--ink-500)", cursor: "pointer", padding: 6 }} aria-label={show ? "Esconder senha" : "Mostrar senha"}>
                ...
                <button type="button" onClick={() => setMode("recover")} style={{ alignSelf: "flex-start", background: "transparent", border: "none", color: "var(--brand)", fontSize: 13, fontWeight: 500, padding: 0, cursor: "pointer" }}>Esqueci a senha</button>
            ...
            <>
              ...
              <p style={{ color: "var(--ink-500)", marginBottom: 28, fontSize: 14 }}>{sent ? "Pronto. Olha sua caixa de entrada." : "A gente te manda um link pra redefinir a senha."}</p>
              ...
              {!sent ? (
                <form onSubmit={(e) => { e.preventDefault(); setSent(true); }} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  ...
                  <button type="button" onClick={() => setMode("login")} style={{ background: "transparent", border: "none", color: "var(--ink-500)", fontSize: 13, cursor: "pointer", marginTop: 4 }}>← Voltar pro login</button>
                </form>
              ) : (
                <div style={{ background: "var(--credit-50)", color: "var(--credit-700)", padding: 16, borderRadius: 12, fontSize: 14, lineHeight: 1.5, border: "1px solid var(--credit-500)" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8, fontWeight: 600 }}><I.CheckCircle size={16}/>Link enviado.</div>
      ...
      {/* Right — brand panel with gramado */}
