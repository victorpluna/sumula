// Relatórios — financial reports (geral / inadimplência / fluxo / extrato).

const Relatorios = ({ data }) => {
  const [tab, setTab] = React.useState("geral");
  const [extratoId, setExtratoId] = React.useState(null);
    ...
    <>
      <div className="page-header">
        ...
        <div className="actions">
          <Button variant="secondary" icon={I.Print}>Imprimir</Button>
          <Button variant="secondary" icon={I.Download}>Baixar PDF</Button>
        </div>
      </div>
...
const RelGeral = ({ recebido, despesas, saldo, pendente, data }) => (
  <>
...
const RelInadimplencia = ({ data, inadimplentes, onExtrato }) => {
  ...
  return (
    <>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, marginBottom: 16 }}>
        ...
        <StatCard label="Mensalistas em dia" value={data.participantes.filter((p) => p.ativo && !data.movs.some((m) => m.participanteId === p.id && (m.status === "pendente" || m.status === "vencido"))).length} sub={`de ${data.participantes.filter((p) => p.ativo).length} ativos`}/>
      ...
      <div className="table-wrap">
            ...
            {inadimplentes.map((p) => {
              ...
              return (
                <tr key={p.id}>
                  ...
                  <td>
                    <Button variant="ghost" size="sm" iconRight={I.ChevronRight} onClick={() => onExtrato(p.id)}>Extrato</Button>
                  </td>
                </tr>
              );
            })}
        ...
        {inadimplentes.length === 0 && <EmptyState icon={I.Trophy} title="Time inteiro em dia." body="Todos os mensalistas estão quites."/>}
      </div>
    </>
  );
};
...
const RelExtrato = ({ data, selectedId, onSelect }) => {
        ...
        <div style={{ maxHeight: 480, overflowY: "auto" }}>
          {data.participantes.map((part, i) => (
            ...
            >
              ...
              {id === part.id && <I.ChevronRight size={14}/>}
            </button>
          ))}
        </div>
      </div>
        ...
        {p && (
          <>
            ...
            <div className="card card-pad-0">
              ...
              <table className="t">
                <thead>
                  <tr><th>Cód.</th><th>Categoria</th><th>Vencimento</th><th>Pagamento</th><th>Status</th><th className="num">Valor</th></tr>
                </thead>
                <tbody>
                  {items.map((m) => {
              ...
              {items.length === 0 && <EmptyState icon={I.Receipt} title="Sem histórico." body="Esse mensalista ainda não tem lançamentos."/>}
            </div>
          </>
        )}
      </div>
