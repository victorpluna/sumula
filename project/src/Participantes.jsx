// Participantes — list + create/edit modal + activate/deactivate.

const Participantes = ({ data, onAdd, onUpdate, onToggleStatus, onDelete }) => {
  ...
  const [form, setForm] = React.useState({ nome: "", valor: "120,00" });
  ...
  const submit = () => {
    ...
    if (editing) {
      ...
      toast({ title: "Participante atualizado.", sub: form.nome.trim(), kind: "success", icon: I.CheckCircle });
    } else {
      onAdd({ nome: form.nome.trim(), valorMensal: valor });
      toast({ title: "Participante cadastrado.", sub: `${form.nome.trim()} entrou no time.`, kind: "success", icon: I.User });
    }
    setModal(false);
  };
    ...
    <>
      <div className="page-header">
        ...
        <div className="actions">
          <Button variant="secondary" icon={I.Download}>Exportar lista</Button>
          <Button icon={I.Plus} onClick={() => open(null)}>Cadastrar participante</Button>
        </div>
      </div>
      ...
      <div className="table-wrap">
                ...
                <tr key={p.id}>
                  ...
                  <td>
                    <div className="row-actions">
                      <Button variant="ghost" size="sm" className="btn-icon" onClick={() => open(p)} title="Editar"><I.Edit size={14}/></Button>
                      <Button
                        variant="ghost"
                        size="sm"
                        className="btn-icon"
                        onClick={() => {
                          setConfirm({
                            ...
                            action: () => {
                              onToggleStatus(p.id);
                              toast({ title: p.ativo ? "Participante desativado." : "Participante reativado.", sub: p.nome, kind: "success", icon: p.ativo ? I.X : I.CheckCircle });
                            },
                          });
                        }}
                        title={p.ativo ? "Desativar" : "Reativar"}
                      >
                        {p.ativo ? <I.X size={14}/> : <I.Check size={14}/>}
                      </Button>
                    </div>
                  </td>
                </tr>
        ...
        {list.length === 0 && (
          <EmptyState
            ...
            action={!search ? <Button size="sm" icon={I.Plus} onClick={() => open(null)}>Cadastrar participante</Button> : null}
          />
        )}
      </div>
      ...
      >
        ...
        <Field
          label="Mensalidade"
          help="Valor padrão das cobranças mensais. Pode ser ajustado por lançamento."
          error={errors.valor}
        >
